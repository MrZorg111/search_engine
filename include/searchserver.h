...
public:
    ...
    int SumCount(const std::vector<Entry>& num);
    ...
    std::vector<std::vector<std::string>> PreRequest(const std::vector<std::string>& request_list);

    std::vector<std::vector<std::pair<int, float>>> PreAnswer(const std::vector<std::vector<RelativeIndex>>& search_result);
};